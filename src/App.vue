...
export default {
  ...
  methods: {
    ...
    markTodoComplete(status){
      let locateIndex = this.todoLists.findIndex(todo => todo.id === status.id);
      if(locateIndex !== -1) {
    ...
    editTodo(newTodo){
      let locateIndex = this.todoLists.findIndex(todo => todo.id === newTodo.id);
      if(locateIndex !== -1) {
        if(!newTodo.isEdit){
...
<template>
  <div class="container mx-auto py-10">
    ...
    <div class="card my-10 max-w-lg mx-auto">
      ...
      <TodoList :lists="todoLists" @updateTodo="markTodoComplete" @deleteTodo="removeFromLists" @enableEdit="editTodo" @updateNewTodo="editTodo" />
    </div>
  </div>
</template>